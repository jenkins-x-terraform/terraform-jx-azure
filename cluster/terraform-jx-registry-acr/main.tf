...
resource "azurerm_resource_group" "acr" {
  count    = var.external_registry_url == "" && var.use_existing_acr_name == null || var.acr_enabled == true ? 1 : 0
...
data "azurerm_container_registry" "acr_existing" {
  count = var.use_existing_acr_name == null || var.acr_enabled == true ? 0 : 1
...
resource "azurerm_role_assignment" "acrpull" {
  count                = var.external_registry_url == "" || var.acr_enabled == true ? 1 : 0
